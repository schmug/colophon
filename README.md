...
from an estimate into a fact you can audit. Colophon is 10/12 open (the index
defines 14 dimensions; the two access-method dimensions don't apply to a local
artifact, so 12 are scored — honestly red on "preprint" and "paper", no free
greens); a typical closed API model is 0/12. The contrast *is* the pitch.
