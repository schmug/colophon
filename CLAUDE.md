...
- **The NumPy MLP is the default and the auditable reference, by choice.** The
  ...
  and Marginalia stays stdlib-only. Note: Marginalia and Incipit are currently
  MLP-only (`marginalia.load_model` expects the MLP's arrays); a transformer
  `.npz` degrades to an unavailable mode — documented in the README.
