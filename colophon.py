...
def train_model(text, stoi, chars, K=12, E=24, H=128, steps=6000, batch=64,
    ...
    if arch == "transformer":
        ...
        manifest = {
            ...
            "loss_history": [[s, l] for s, l in hist],
...
def generate(p, stoi, itos, K, prompt="", n=240, temp=0.8, seed=0,
    ...
    Unknown prompt characters map to PAD (id 0), the same convention as
    prompt_confidence()/inspect_prompt() -- the model has no representation
    for them, and the records must describe the context the sampler actually
    used."""
    rng = np.random.default_rng(seed)
    ctx = [0] * K
    for ch in prompt:
